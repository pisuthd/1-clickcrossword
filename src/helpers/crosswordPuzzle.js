function CrosswordPuzzle({
    ...
    let addWord = function (word) {
        ...
        for (let letterIndex = 0; letterIndex < word.text.length; ++letterIndex) {
            ...
            if (word.vertical) {
                row += letterIndex;
                positions.push(((word.row + letterIndex) * gridSize) + word.column )
            }
            else {
                column += letterIndex;
                positions.push((word.row * gridSize) + word.column + letterIndex)
            }
        ...
        words.push({
            word: word.text,
            startPosition: (word.row * gridSize) + word.column,
            positions
        })
    }
    ...
    const flatten = () => {
        return grid.reduce((arr, item) => arr.concat(item), [])
    }
